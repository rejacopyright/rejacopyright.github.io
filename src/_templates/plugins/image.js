...
function Index(props){
  const [src, srcSet] = useState(require(`${process.env.REACT_APP_IMAGES}/sample/1.jpg`).default)
        ...
        <div className="col-md-4 center">
          <div className="row">
            <div className="col-12"> <h5 className="m-0 lh-75 text-dark">Example Image</h5> <hr className="my-2"/> </div>
            {
              Array(2).fill().map((r, key) => (
                <div className="col-6" key={key}>
                  <img className="w-100 pointer radius-5 shadow" src={`${require(`${process.env.REACT_APP_IMAGES}/sample/${key+2}.jpg`).default}?${Date.now()}`} alt="" onClick={gambarIni} />
                </div>
              ))
            }
            <div className="col-12 mt-5"> <h5 className="m-0 lh-75 text-dark">Example Modal</h5> <hr className="my-2"/> </div>
            {
              Array(2).fill().map((r, key) => (
                <div className="col-6" key={key}>
                  <img modal="true" className="w-100 pointer radius-5 shadow" src={`${require(`${process.env.REACT_APP_IMAGES}/sample/${key+9}.jpg`).default}?${Date.now()}`} alt="" onClick={gambarIni} />
                </div>
              ))
            }
          </div>
